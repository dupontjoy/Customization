...
set "api_url=https://api.github.com/repos/xiaoyifang/goldendict-ng/releases/latest"
set "file_pattern=GoldenDict-ng-.*-Qt6.7.2-Windows-installer\.7z"

:: 获取下载链接
for /f %%i in ('powershell -Command "(Invoke-WebRequest -Uri '%api_url%' -UseBasicParsing | ConvertFrom-Json).assets ^| Where-Object { $_.name -match '%file_pattern%' } ^| Select-Object -First 1 | ForEach-Object { $_.browser_download_url }"') do (
    set "original_url=%%i"
)
