...
:: 版本文件
set "version_file=versions_gitextensions.txt"
...
call :check_version
if "%need_update%"=="1" (
    call :update_gitextensions
    call :unzip_gitextensions
...
echo.&echo █ 正在检查gitextensions版本...

:: GitHub API 地址
set "api_url=https://api.github.com/repos/gitextensions/gitextensions/releases/latest"
...
:update_gitextensions
setlocal enabledelayedexpansion
echo.&echo █ 正在更新gitextensions...

:: GitHub API 地址和文件名匹配模式
set "api_url=https://api.github.com/repos/gitextensions/gitextensions/releases/latest"
set "file_pattern=gitextensions-.*-x64-.*\.zip"
...
powershell -Command "$maxRetry=3; $retryCount=0; do { try { Invoke-WebRequest -Uri '%download_url%' -OutFile '%cd%\gitextensions-latest.zip' -TimeoutSec 30; break } catch { $retryCount++; if ($retryCount -ge $maxRetry) { throw }; Start-Sleep -Seconds 5 } } while ($true)"
...
tar -xf .\gitextensions-latest.zip
del /s /q .\gitextensions-latest.zip
goto :eof
